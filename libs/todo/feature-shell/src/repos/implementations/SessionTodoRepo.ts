...
export class SessionTodoRepo implements ITodoRepo {
  static STORAGE_KEY = "todo-app-todos";

  public async getAllTodo(): Promise<Todo[]> {
    const todo = sessionStorage.getItem(SessionTodoRepo.STORAGE_KEY);
  ...
  public async save(todo: Todo): Promise<void> {
    ...
    sessionStorage.setItem(SessionTodoRepo.STORAGE_KEY, rawTodoAsString);
  }
}
