...
import { SessionTodoRepo } from "./SessionTodoRepo";
...
describe("SessionTodoRepo", () => {
  let todoRepo: SessionTodoRepo;

  beforeEach(() => {
    todoRepo = new SessionTodoRepo();
    sessionStorage.setItem(
      SessionTodoRepo.STORAGE_KEY,
  ...
  describe("save", () => {
    it("should convert todo to persistance and prepend it to the list", async () => {
      ...
      const newTodoList = JSON.parse(
        sessionStorage.getItem(SessionTodoRepo.STORAGE_KEY)!
      );
